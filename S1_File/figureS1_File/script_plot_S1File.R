...
host="comparing_host_excusion.csv"
data_host <- readr::read_csv(host)

torque_data <- data_host %>% filter(grepl("Torque teno virus", tax_name, fixed = TRUE)) %>% group_by(sample, technique) %>% summarise(abundance = sum(abundance), pct=sum(pct))
